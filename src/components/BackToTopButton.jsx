...
const BackToTopButton = ({ extraMargin = false, visible }) => {
    ...
    return <button onClick={clickHandler} className={`fixed flex w-fit items-center justify-center gap-1 right-6 ${visible ? "bottom-24 lg:bottom-28" : "bottom-5"} bg-[#03336698] border-2 border-blue-400 text-white py-1 lg:py-1.5 px-2 rounded-md lg:text-sm cursor-pointer transform transition-all duration-300 ease-linear font-semibold ${showBtn ? "translate-y-o" : "translate-y-[200%] z-50"}`}>
